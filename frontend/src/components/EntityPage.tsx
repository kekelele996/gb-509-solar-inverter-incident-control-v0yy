...
export function EntityPage({ config, useStore }: { config: EntityConfig; useStore: EntityStore }) {
  const { items, meta, loading, error, interlock, load, createRecord, transition, confirmInterlock, clearInterlock } = useStore();
  ...
  const isInterlockConfirm = config.key === 'mitigationAction' && pending?.status === 'executing';
  ...
  const finalizeTransition = async () => {
    if (!pending) return;
    try {
      if (isInterlockConfirm) await confirmInterlock(config.path, pending.item);
      else await transition(config.path, pending.item, pending.status);
      setPending(null); setRemoteConfirm(false);
    }
  ...
  return <main className="workspace">
    ...
    {interlock && <section className="interlock-panel" role="status" aria-label="故障联锁确认结果">
      <header><strong>故障联锁确认完成</strong><button className="link-button" onClick={clearInterlock}>关闭</button></header>
      <dl>
        <div><dt>处置动作</dt><dd>{interlock.action.code} · {interlock.action.name}</dd><StatusBadge status={interlock.action.status} /></div>
        <div><dt>关联故障事件</dt><dd>{interlock.fault.code} · {interlock.fault.name}</dd><StatusBadge status={interlock.fault.status} /></div>
        <div><dt>关联逆变器</dt><dd>{interlock.inverter.code} · {interlock.inverter.name}</dd><StatusBadge status={interlock.inverter.status} /></div>
      </dl>
    </section>}
    ...
    <ActionDrawer open={remoteConfirm} item={pending?.item || null} target={pending?.status || ''} hint={isInterlockConfirm ? '确认后故障联锁生效：关联故障事件转为已缓解，关联逆变器转为隔离，三者与审计一并落盘；任一失败全部回滚。' : ''} onCancel={() => setRemoteConfirm(false)} onConfirm={() => void finalizeTransition()} />
  </main>;
}
